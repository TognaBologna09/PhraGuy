...
public class AnimFootNetwork : NetworkBehaviour
{
    [Header("Player")]
    [Tooltip("A reference to the Player Transform with collider.")]
    public Transform bodyRoot;
    public Rigidbody playerRigidbody;
    public AnimFootNetwork otherFoot;
    ...
    private void FixedUpdate()
    {
        ...
        //
        // Transform to current location always, and assign current location to new or old position
        //
        // old -=> current  
        //              currentFarEnough? current = new : current = old   
        //  
        ...
        // Now search for a new point in the expected foot area
        ...
        // Now search for a new point in the unexpected foot area
        
        else
        {

            Vector3 newOrigin = ray.GetPoint(0.9f*legHeight);                      // store the point where the last raycast would have polled
            Vector3 newDirection = (newOrigin - new Vector3(bodyRoot.position.x, bodyRoot.position.y - comHeight, bodyRoot.position.z)).normalized;    // shoot a ray towards the COM of the frog 
            Ray nray = new Ray(newOrigin, newDirection);    

            if(Physics.Raycast(nray, out RaycastHit infoi, 1.4f*legHeight, stepLayer))
            {
                debugVector = info.point;
                if (isDebugMode)
                {
                    Debug.DrawRay(newOrigin, newDirection * 2*legHeight, Color.magenta);
                    Debug.DrawRay(oldPosition, (info.point - oldPosition).normalized * distance, Color.red);
                }
                ...
                if (Vector3.Distance(oldPosition, info.point) > stepDistance && time >= 1 && !otherFoot.IsMoving())
                {
                    time = 0;
                    int direction = bodyRoot.InverseTransformPoint(info.point).z > bodyRoot.InverseTransformPoint(newPosition).z ? 1 : -1;
                    newPosition = infoi.point;// + (bodyRoot.forward * stepDistance * direction);
                    newNormal = infoi.normal;
                }
            }
        }
